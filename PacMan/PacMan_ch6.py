...
def collision(etat):
    for fantome in fantomes:
        if (fantome.rect().colliderect(pacman.rect())):
            if pacman.super > 0:
...
def affiche_message(surface):
    if etat != "joue":
        text = font.render(etat, True, (255, 255, 255))
        pos_text = text.get_rect(center=(width//2, height//2))
...
while terminé == False:
    ...
    if etat == "joue":
        for i in range(4):
            ...
            anime_fantomes()
            etat = collision(etat)
    etat = teste_gagne(etat)
    dessine_laby(screen)
    dessine_sprites(screen)
    affiche_message(screen)
    pygame.display.flip()
pygame.quit()
